...
jQuery(function($){
    ...
    var BlogPrimeNonce = blog_prime_admin.ajax_nonce;
    var BlogPrimeAjaxUrl = blog_prime_admin.ajax_url;
    $('.twp-custom-setup').click(function(){
        jQuery.post(BlogPrimeAjaxUrl, {
            action: 'blog_prime_notice_dismiss',
            security : BlogPrimeNonce,
        });
        $('.twp-blog-prime-notice').hide();
    ...
    $('.twp-install-active').click(function(){
        ...
        $(this).closest('.twp-blog-prime-notice').addClass('twp-installing');

        var data = {
            'action': 'blog_prime_install_plugins',
            '_wpnonce': BlogPrimeNonce,
        };
    ...
    $('.theme-recommended-plugin .recommended-plugin-status').click(function(){
        ...
        $.post(ajaxurl, data, function( response ) {
            ...
            var active = blog_prime_admin.active
            var deactivate = blog_prime_admin.deactivate
